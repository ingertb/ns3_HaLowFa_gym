...
MacLow::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MacLow")
    ...
    .AddTraceSource ("DataAck", "The STA has received an Ack to its data frame.",
                     MakeTraceSourceAccessor (&MacLow::m_dataAckLogger),
                     "ns3::Mac48Address::TracedCallback")
  ;
  return tid;
}
...
uint32_t
MacLow::GetAssRespAck(void)
{
  return m_ass;
}

uint32_t
MacLow::GetAuthRespAck(void)
{
  return m_auth;
}

uint32_t 
MacLow::GetAuthRpS (void)
{
...
uint32_t 
MacLow::GetAssRpS (void)
{
  uint32_t tmp = contAssResp;
  contAssResp = 0;
  return tmp;
}

uint32_t 
MacLow::GetAuthRpA (void)
{
  uint32_t tmp = m_auth;
  m_auth = 0;
  return tmp;
