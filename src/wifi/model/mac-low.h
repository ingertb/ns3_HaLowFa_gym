...
public:
  ...
  //************************************
  //HaLow 
  uint32_t GetAuthRespAck (void); //number of authentication responses sended and acknowledged
  uint32_t GetAssRespAck(void);  //number of association responses sended and acknowledged 
  uint32_t GetAuthRpS (void); //number of authentication responses sended (interval)
  uint32_t GetAssRpS (void); //number of association responses sended (interval)
  uint32_t GetAuthRpA (void); //number of authentication responses sended and acknowledged (interval)
  uint32_t GetAssRpA (void); //number of association responses sended and acknowledged (interval)
...
private:
  ...
  TracedCallback<Mac48Address> m_dataAckLogger; //!< Logs events when the STA gets an ACK for a data frame
};
