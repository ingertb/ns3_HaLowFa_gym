...
public:
    ...
    /**
     * set the status of collision occurring in the preamble and header duration
     */
   void SetPreambleAndHeaderCollisionStatus (bool status);
    /**
     * return the status of collision occurring in the preamble and header duration
     */
   bool GetPreambleAndHeaderCollisionStatus (void);
    /**
     * set the status of collision occurring in the payload duration
     */
   void SetPayloadCollisionStatus (bool status);
    /**
     * return the status of collision occurring in the payload duration
     */
   bool GetPayloadCollisionStatus (void);
...
private:
    ...
    Time m_endTime;
    double m_rxPowerW;
    bool m_preambleAndHeaderCollisionStatus;
    bool m_payloadCollisionStatus; 
  };
  ...
  /**
   * Calculate the sinr of an event at a defined time
   * \param event
   * \param time
   */
  double CalculateSinrAtTime (Ptr<Event> event, Time time);
  /**
   * Check if the event overlaps with the provided time
   */
  bool IsOverlapWithTime (Ptr<InterferenceHelper::Event> event, Time time);
 
  /*
   * Frame capture success rate calculation
   */
   double CalculateDataCaptureSuccessProbability (Ptr<InterferenceHelper::Event> event, double sinr,
                  Time duration, WifiMode mode, WifiTxVector txVector);
   double CalculateDataRecoverySuccessProbability (Ptr<InterferenceHelper::Event> event, double sinr,
                   Time duration, WifiMode mode, WifiTxVector txVector);
   double CalculatePreambleAndPlcpCaptureSuccessProbability (Ptr<InterferenceHelper::Event> event,
                 double sinr, Time duration, WifiMode mode, WifiTxVector txVector);
   double CalculatePreambleAndPlcpRecoverySuccessProbability (Ptr<InterferenceHelper::Event> event,
                   double sinr, Time duration, WifiMode mode, WifiTxVector txVector);
...
public:
    ...
    /**
     * Get events associated with this niChange
     */
...
private:
  ...
  Ptr<FrameCaptureModel> m_frameCaptureProbCalc;
