...
YansWifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::YansWifiPhy")
    ...
    .AddAttribute ("ChannelWidth", "Whether 1MHz, 2MHz, 4MHz, 8MHz, 16MHz, 20MHz or 40MHz.",
                   ...
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("EnablePreambleCapture", "Activate preamble capture effect.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&YansWifiPhy::m_doPreambleCapture),
                   MakeBooleanChecker ())
    .AddAttribute ("EnableDataCapture", "Activate data capture effect.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&YansWifiPhy::m_doDataCapture),
                   MakeBooleanChecker ()) 
      .AddAttribute ("SinrDiffCapture",
                   "sinr difference for packet capturing and  recovery (dB).",
                   DoubleValue (10.0),
                   MakeDoubleAccessor (&YansWifiPhy::SetSinrDiffCapture,
                                       &YansWifiPhy::GetSinrDiffCapture),
                   MakeDoubleChecker<double> ())
  ;
  return tid;
}
...
void
YansWifiPhy::SetSinrDiffCapture (double sinr)
{
  NS_LOG_FUNCTION (this << sinr);
  SinrDiffCapture = sinr;
}
...
double
YansWifiPhy::GetSinrDiffCapture (void) const
{
  return SinrDiffCapture;
}
                                            ...
                                            uint8_t packetType, Time rxDuration)
{
  ...
  event = m_interference.Add (packet->GetSize (),
                              ...
                              rxDuration,
                              rxPowerW);
  double sinrAtTime = m_interference.CalculateSinrAtTime (event, Simulator::Now());
  WifiMode plcpHeaderMode = WifiPhy::GetPlcpHeaderMode(event->GetPayloadMode(), preamble);
  Time currentStage = Seconds (0);
    ...
    case YansWifiPhy::RX:
      {
        ...
        currentStage = Simulator::Now () - m_rxPacketEvent->GetStartTime();
        //double oldsinr = m_interference.CalculateSinrAtTime (m_rxPacketEvent, Simulator::Now());
        double oldsinr;
        if (currentStage <= preambleAndHeaderDuration) // receive preamble
 			{
			  NS_LOG_DEBUG (Simulator::Now () << ", old event start_time="<<m_rxPacketEvent->GetStartTime()
					 <<", end_time="<<m_rxPacketEvent->GetEndTime()
					 <<", power="<<m_rxPacketEvent->GetRxPowerW());
                          oldsinr = m_interference.CalculateSinrAtTime (m_rxPacketEvent, Simulator::Now());
			  double tempPER = 1.0 - m_interference.CalculatePreambleAndPlcpRecoverySuccessProbability (m_rxPacketEvent, oldsinr, preambleAndHeaderDuration - currentStage,
									plcpHeaderMode, txVector);
			  m_rxPacketEvent->SetPreambleAndHeaderCollisionStatus (true);
			  NS_LOG_DEBUG (Simulator::Now ()<< ", old_sinr="<<oldsinr <<", tempPER="<<tempPER);
			  if (m_random->GetValue () > tempPER)
		    {
		      NS_LOG_DEBUG (Simulator::Now()<<" preamble capture: dropping newly arrived packet (newSinr="<<RatioToDb(sinrAtTime)<<"dB)");
		      NotifyRxDrop (packet);
		      if (endRx > Simulator::Now() + m_state->GetDelayUntilIdle())
					{
					  goto maybeCcaBusy;
		    ...
		    {
		      NS_LOG_DEBUG (Simulator::Now()<<" preamble capture: dropping current rxed packet (oldsinr="<<RatioToDb(oldsinr)<<"dB)");
		      // check if endRxEvent and endPlcpRxEvent have been scheduled
		      if (m_endPlcpRxEvent.IsRunning())
						m_endPlcpRxEvent.Cancel ();
		      if (m_endRxEvent.IsRunning())
						m_endRxEvent.Cancel ();
                      NotifyRxDrop (m_rxPacket);
		      m_interference.NotifyRxEnd();
		      m_state->SwitchFromRxAbort ();
		      m_rxPacket = NULL;
		      m_rxPacketEvent = NULL;

		      if (m_doPreambleCapture)
					{
                                        double sinrAtTimeForPreambleCapture = sinrAtTime / pow(10, SinrDiffCapture/10);
				  	double tempPER = 1.0 - m_interference.CalculatePreambleAndPlcpCaptureSuccessProbability (event, sinrAtTimeForPreambleCapture, preambleAndHeaderDuration,
													     																																				plcpHeaderMode, txVector);
					  if (m_random->GetValue() > tempPER)
					  {
				      NS_ASSERT (m_endPlcpRxEvent.IsExpired() && m_endRxEvent.IsExpired());
                                     NS_LOG_DEBUG (Simulator::Now()<< " preamble capture: switch rx to new packet (newsinr="<<RatioToDb(sinrAtTime)<<")");
                                     // sync to the new signal
				      m_state->SwitchToRx (rxDuration);
				      m_rxPacket = packet;
				      m_rxPacketEvent = event;
				      // start to receive the new packet
				      ...
				      m_endPlcpRxEvent = Simulator::Schedule (preambleAndHeaderDuration, &YansWifiPhy::StartReceivePacket, this,
                                      ...
                                      m_rxDurationOfCapturedPacket = m_rxDuration;
					  }
					  else
					  {
				      NS_LOG_DEBUG (Simulator::Now()<<" preamble capture: signal is too low for capture (newsinr="<<RatioToDb(sinrAtTime)<<"dB)");
				      NotifyRxDrop (packet);
				      if (endRx > Simulator::Now() + m_state->GetDelayUntilIdle())
							{
							  goto maybeCcaBusy;
					...
					{
					  NS_LOG_DEBUG (Simulator::Now() << " drop packet because already receiving (sinr=" << RatioToDb(sinrAtTime) << "dB)");
					  NotifyRxDrop (packet);
					  if (endRx > Simulator::Now() + m_state->GetDelayUntilIdle())
				    {
				      goto maybeCcaBusy;
      ...
      else // when a new packet arrives, the receiver is receiving the payload part of the old packet
			{
                          oldsinr = m_interference.CalculateSinrAtTime (m_rxPacketEvent, Simulator::Now());
			  double tempPER = 1.0 - m_interference.CalculateDataRecoverySuccessProbability (m_rxPacketEvent, oldsinr, rxDuration - currentStage,
												       																													plcpHeaderMode, txVector);
			  m_rxPacketEvent->SetPayloadCollisionStatus (true);
			  NS_LOG_DEBUG(Simulator::Now() << " payload capture: oldsinr is "<<oldsinr<<", tempPER is "<<tempPER);
			  if (m_random->GetValue () > tempPER) //  if the currently received packets can still be decoded
		    {
		      NS_LOG_DEBUG (Simulator::Now() << " data capture: drop packet because already synced to stronger signal (oldsinr=" << RatioToDb(oldsinr) << "dB)");
		      NotifyRxDrop (packet);
		      if (endRx > Simulator::Now () + m_state->GetDelayUntilIdle ())
					{
			  ...
			  {
		      NS_LOG_DEBUG (Simulator::Now() << " data capture: dropping currently rxed packet with oldsinr=" << RatioToDb(oldsinr) << "dB");
		      // abort old rx
		      //NS_ASSERT (m_endPlcpRxEvent.IsExpired ());
		      //m_endRxEvent.Cancel();
                      if (m_endPlcpRxEvent.IsRunning())
		      ...
		      m_rxPacketEvent = NULL;
			    ...
			    if (m_doDataCapture)  // if the data capture is enabled
					{
                                          double sinrAtTimeForPreambleCapture = sinrAtTime / pow (10, SinrDiffCapture/10);
					  double tempPER = 1.0 - m_interference.CalculateDataCaptureSuccessProbability (event, sinrAtTimeForPreambleCapture, rxDuration - currentStage,
														  plcpHeaderMode, txVector);
					  if (m_random->GetValue() > tempPER) // if the newly arrived signal is strong enough to be captured
					  {
				      NS_LOG_DEBUG (Simulator::Now() << " data capture: switching rx to new packet with sinr=" << RatioToDb(sinrAtTime) << "dB");
				      // sync to new signal
				      m_state->SwitchToRx (rxDuration);
				      m_rxPacket = packet;
				      m_rxPacketEvent = event;
				      // start to receive the newly arrived packet
				      NotifyRxBegin (packet);
				      m_interference.NotifyRxStart ();
				  	...
				  	else  // if the newly signal is too weak to be captured
				    {
				      NS_LOG_DEBUG (Simulator::Now() << " data capture: signal is too low for capture with sinr=" << RatioToDb(sinrAtTime) << "dB");
				      NotifyRxDrop (packet);
				      if (endRx > Simulator::Now () + m_state->GetDelayUntilIdle ())
							{
							  // that packet will be noise _after_ the reception of the
				  ...
				  else // if the data capture is not enabled
					{
					  NS_LOG_DEBUG (Simulator::Now() << " drop packet because already receiving (power=" << rxPowerW << "W)");
					  NotifyRxDrop (packet);  // drop the newly arrived packets and thus both packets are dropped
					  if (endRx > Simulator::Now () + m_state->GetDelayUntilIdle ())
				    {
				      // that packet will be noise _after_ the reception of the
				      // currently-received packet.
...
maybeCcaBusy:
  ...
  Time delayUntilCcaEnd = m_interference.GetEnergyDuration (m_ccaMode1ThresholdW);
  if (m_plcpSuccess == true && currentStage >= preambleAndHeaderDuration)
    {
      if (delayUntilCcaEnd >= (rxDuration - currentStage))	// if the duration of CCA_BUSY is longer than the reception duration
        {
	  m_state->SwitchMaybeToCcaBusy (delayUntilCcaEnd);
        }
      else
       {
         delayUntilCcaEnd = rxDuration - currentStage;		// if the duration of CCA_BUSY is shorter than the reception duration, the rest of reception
								// duration should be declared as CCA_BUSY
         m_state->SwitchMaybeToCcaBusy (delayUntilCcaEnd);
       }
    }
  else
                                 ...
                                 Ptr<InterferenceHelper::Event> event)
{
  ...
  snrPer.snr = m_interference.CalculateSinrAtTime(event, Simulator::Now());
  snrPer.per = 0.0;
  //if (!event->GetPreambleAndHeaderCollisionStatus())
  //  {
      snrPer.per = 1.0 - m_interference.CalculatePreambleAndPlcpRecoverySuccessProbability(event, snrPer.snr, 
                      CalculatePlcpPreambleAndHeaderDuration (txVector, preamble),
                      WifiPhy::GetPlcpHeaderMode(event->GetPayloadMode(), preamble), txVector);
   // }

  NS_LOG_DEBUG ("snr=" << snrPer.snr << ", per=" << snrPer.per);

  if (m_random->GetValue () > snrPer.per && preamble != WIFI_PREAMBLE_NONE)   //plcp reception succeeded
    {
      /*
      ...
      if (IsModeSupported (txMode) || IsMcsSupported (txMode))
        {
          ...
          NS_ASSERT (m_endRxEvent.IsExpired ());
          Time preambleAndHeaderDuration = CalculatePlcpPreambleAndHeaderDuration (txVector, preamble);
          m_endRxEvent = Simulator::Schedule (m_rxDurationOfCapturedPacket - preambleAndHeaderDuration, &YansWifiPhy::EndReceive, this,        
                                              packet, preamble, packetType, event);
          //std::cout<<Simulator::Now()<<" packetId= "<<packet->GetUid()<<" preambleSinr= "<<RatioToDb(snrPer.snr) <<" psr= "<<1.0 - snrPer.per<<std::endl;          
        }
      else //mode is not allowed
        {
         NS_LOG_DEBUG ("drop packet because it was sent using an unsupported mode (" << txMode << ")");
...
YansWifiPhy::EndReceive (Ptr<Packet> packet, enum WifiPreamble preamble, uint8_t packetType, Ptr<InterferenceHelper::Event> event)
{
  ...
  snrPer.snr = m_interference.CalculateSinrAtTime(event, Simulator::Now());
  snrPer.per = 0.0;
  //if (!event->GetPayloadCollisionStatus())
  //  {
       WifiMode plcpHeaderMode = WifiPhy::GetPlcpHeaderMode (event->GetPayloadMode(), event->GetPreambleType());
       snrPer.per  = 1.0 - m_interference.CalculateDataRecoverySuccessProbability  (event, snrPer.snr, event->GetDuration(),									     																														plcpHeaderMode, event->GetTxVector());
   // }
  m_interference.NotifyRxEnd ();
      ...
      if (m_random->GetValue () > snrPer.per)
        {
          ...
          //std::cout<<Simulator::Now()<<" packetId= "<<packet->GetUid()<<" payloadSinr= "<< RatioToDb (snrPer.snr)  <<" psr= "<< 1.0 - snrPer.per<<std::endl;
        }
      else
        {
          /* failure. */
